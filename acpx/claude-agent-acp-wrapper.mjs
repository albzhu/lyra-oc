...
const stderrLogFileNamePrefix = "claude-agent-acp-wrapper.stderr";
...
// Resolve the installed Claude Agent ACP binary dynamically instead of
// hardcoding a machine-specific path. Order: explicit env override -> Node
// module resolution -> scan ~/.openclaw/npm/projects/*/.../claude-agent-acp/dist.
// Returns undefined when nothing is found so the npmCliPath / npx fallbacks
// below take over.
function resolveInstalledBinPath() {
  const PKG = "@agentclientprotocol/claude-agent-acp";
  const BIN_REL = path.join("dist", "index.js");

  const override = (process.env.OPENCLAW_CLAUDE_AGENT_ACP_BIN || "").trim();
...
if (installedBinPath) {
  ...
  defaultCommand = process.execPath;
  defaultArgs = [npmCliPath, "exec", "--yes", "--package", "@agentclientprotocol/claude-agent-acp@0.39.0", "--", "claude-agent-acp"];
} else {
  defaultCommand = process.platform === "win32" ? "npx.cmd" : "npx";
  defaultArgs = ["--yes", "--package", "@agentclientprotocol/claude-agent-acp@0.39.0", "--", "claude-agent-acp"];
}
const command =
